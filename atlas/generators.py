...
def compile_op(op: Callable, gen: 'Generator', sid: str, hooks: List[Hook]) -> Callable:
    """
    ...
    def create_op(*args, **kwargs):
        for h in hooks:
            h.before_op(*args, **kwargs, generator=gen, sid=sid)

        result = op(*args, **kwargs)

        for h in hooks:
            h.after_op(*args, **kwargs, generator=gen, sid=sid, retval=result)
...
def compile_func(gen: 'Generator', func: Callable, strategy: Strategy, hooks: List[Hook] = None) -> Callable:
    ...
    for n in astutils.preorder_traversal(f_ast):
        if isinstance(n, ast.Call) and isinstance(n.func, ast.Name) and n.func.id in known_ops:
            ...
            ops[n.func.id] = compile_op(op, gen, sid, hooks)
