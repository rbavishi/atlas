...
class TestGeneratorInversionStrategy(GeneratorInversionStrategy):
...
def simple_enumerator(inputs, output, func_seq):
    prog = []
    intermediates = []
    for idx, func in enumerate(func_seq):
        func = api_gens[func]
        try:
            val, args = func(intermediates + inputs, output, idx=idx)
    ...
    return intermediates[-1], prog, intermediates


class TestGeneratorInversion(unittest.TestCase):
    def check(self, inputs: List[Any], output: Any, funcs: List[str], seqs: List[List[int]],
        ...
        found_reproducible_solution = False
        found_solution = False
        for func_seq in func_seqs:
            for val, prog, intermediates in simple_enumerator.generate(inputs, output, func_seq):
                if not checker(output, val):
                    continue
                ...
                for new_val, _, _ in simple_enumerator.with_env(strategy=s).generate(inputs, output, func_seq):
                    ...
                    found_reproducible_solution = True
                    break

                break

        self.assertTrue(found_solution, "Did not find a solution")
        self.assertTrue(found_reproducible_solution, "Could not reproduce output")
    ...
    def test_df_select_dtypes_2(self):
        inputs = [pd.DataFrame({
            'A': 1.0,
            'B': pd.Timestamp('20130102'),
            'C': pd.Series(1, index=list(range(4)), dtype='float32'),
